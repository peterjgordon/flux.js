...
$( document ).ready(function() {
    ...
    $('.sidebar-wrapper').on('click', '.btn-download', function(){
    	...
    	smokyBG.data('flux').download(width, height);
    	return false;
	});
    ...
    $('.sidebar-wrapper').on('click', '.btn-record', function(){
		...
		smokyBG.data('flux').toggleCapture(enable);
	...
	//Ratio slider
	$('.ratio-slider').parents('.input-group').find('.value').html(smokyBG.data("flux").settings.displayRatio);
	var ratioSlider = $('.ratio-slider').noUiSlider({
		start: [ smokyBG.data("flux").settings.displayRatio*10 ],
		range: {
			'min': [ 0 ],
			'max': [ 50 ]
		},
	...
	$("#path").on("change", function(event) {
		smokyBG.data('flux').setPath(this.value);
	});
	...
	function setNewParams(){
		...
		smokyBG.data('flux').setOptions(params);
	}
