...
$( document ).ready(function() {
    ...
    $('.sidebar-wrapper').on('click', '.btn-record', function(){
		...
		smokyBG.data('waterpipe').toggleCapture(enable);
	...
	$("#path").on("change", function(event) {
		smokyBG.data('waterpipe').setPath(this.value);
	});
	...
	function setNewParams(){
    	...
    	var radius = $(window).height()/5*(params.radiusSize/100);
		...
		smokyBG.data('waterpipe').setOption('speed', params.speed);
		smokyBG.data('waterpipe').setOption('fadeSpeed', params.fadeSpeed);
		smokyBG.data('waterpipe').setOption('mousePower', params.mousePower);
		smokyBG.data('waterpipe').setOption('replayPower', params.replayPower);
