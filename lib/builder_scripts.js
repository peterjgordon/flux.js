...
$( document ).ready(function() {
	...
	var smokyBG = $('#wavybg-wrapper').waterpipe({
		displayRatio: parseFloat(getQueryVariable("ratio"))/10 || 1
	});
	...
	//Ratio slider
	$('.ratio-slider').parents('.input-group').find('.value').html(smokyBG.data("waterpipe").settings.displayRatio);
	var ratioSlider = $('.ratio-slider').noUiSlider({
		start: [ smokyBG.data("waterpipe").settings.displayRatio*10 ],
		range: {
			'min': [ 0 ],
			'max': [ 50 ]
		},
	...
	function setNewParams(){
		...
		smokyBG.data('waterpipe').setOptions(params);
	}
